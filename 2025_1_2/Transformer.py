...
#             _, idx = knn(coor_k, coor_q)  # bs k np
    ...
    def forward(self, x):
        ...
        coor_q, f_q = fps_downsample(coor, f, 512)
        ...
        coor_q, f_q = fps_downsample(coor, f, 128)
        f = self.get_graph_feature(coor_q, f_q, coor, f)
        f = self.layer4(f)
        f = f.max(dim=-1, keepdim=False)[0]
        coor = coor_q
...
class CrossAttention(nn.Module):
    def __init__(self, dim, out_dim, num_heads=4, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads
        self.dim = dim
...
class PCTransformer(nn.Module):
    ...
    """
    # depth=[6, 6] in the original...
    def __init__(self, in_chans=3, embed_dim=384, depth=[6, 8], num_heads=6, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        print(f' Transformer with knn_layer {self.knn_layer}.')
...
class SimplePCTransformer(nn.Module):

    """ Lightweight version of PCTransformer — uses DGCNN feature extraction,
        keeps positional embeddings, removes transformers.
    """
    def __init__(self, in_chans=3, embed_dim=384, num_query=224, knn_layer=-1):
        super().__init__()
        self.embed_dim = embed_dim
        self.num_query = num_query
        self.knn_layer = knn_layer

        print(f"SimplePCTransformer initialized (no transformer blocks, knn_layer={knn_layer})")

        # --- DGCNN-style feature grouping ---
        self.grouper = DGCNN_Grouper()  # assumed available in your codebase

        # --- Positional embedding (same as original) ---
        self.pos_embed = nn.Sequential(
            nn.Conv1d(in_chans, 128, 1),
            nn.BatchNorm1d(128),
            nn.LeakyReLU(0.2),
    ...
    def forward(self, inpc):
        """
        Args:
            inpc (B, N, 3): input incomplete point cloud
        Returns:
            q (B, M, C): query features
            coarse_point_cloud (B, M, 3): coarse predicted shape
        """
        B = inpc.size(0)

        # --- DGCNN grouping ---
        coor, f = self.grouper(inpc.transpose(1, 2).contiguous())  # coor: B 3 N, f: B 128 N

        # --- Positional and feature projection ---
        pos = self.pos_embed(coor)  # B C N
        x = self.input_proj(f) + pos  # simple addition instead of transformer fusion

        # --- Global pooling ---
        global_feature = self.increase_dim(x).max(dim=-1)[0]  # B 1024

        # --- Coarse shape prediction ---
        coarse_point_cloud = self.coarse_pred(global_feature).reshape(B, self.num_query, 3)
        ...
        q = self.query_embed(query_feature.transpose(1, 2)).transpose(1, 2)  # B M C
