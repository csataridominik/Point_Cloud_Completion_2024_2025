...
def main(encode = 'hilbert'):
    ...
    if encode == 'hilbert':
        encoded = encode_hilbert(xyz, space_size=1024, convention="xyz")
    elif encode == 'z':
        encoded = encode_z(xyz, space_size=1024, convention="xyz")

    sorted_encoded, sort_idx = torch.sort(encoded, dim=1)
    sorted_xyz = torch.gather(xyz, 1, sort_idx.unsqueeze(-1).expand(-1, -1, 3))

    return sorted_xyz

if __name__ == "__main__":
    encoded = main('hilbert')
